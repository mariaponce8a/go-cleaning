...
export class ListarPedidosComponent implements OnInit, OnDestroy {
  ...
  async generatePDF(id_pedido_cabecera: number) {
    if (!id_pedido_cabecera) {
      console.error('ID de pedido no proporcionado');
      return;
    }

    this.requestService.get(`${Constantes.apiOrdenPedido}/${id_pedido_cabecera}`).subscribe(async data => {
      const doc = new jsPDF();
      doc.setFont('Courier');
      doc.setFontSize(16);
      ...
      if (isPlatform('android') || isPlatform('ios')) {
        try { 
          ...
          const result = await Filesystem.writeFile({
            path: `pedido_${id_pedido_cabecera}.pdf`,
            data: base64Data, 
